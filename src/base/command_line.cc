...
std::basic_string<CharT> ToLowerASCII(T str) {
...
std::wstring QuoteForCommandLineToArgvWInternal(
  ...
  (arg.size() != 2 || arg[0] != L'%' || allow_unsafe_insert_sequences);
...
void CommandLine::AppendSwitchNative(StringPiece switch_string,
  ...
  if (!value.empty()) {
    ...
    combined_switch_string += StringType(value);
  }
...
void CommandLine::RemoveSwitch(base::StringPiece switch_key_without_prefix) {
  ...
  ToLowerASCII(switch_key_without_prefix);
  // GetSwitchPrefixLength(switch_key_native);
    ...
    if (parse_switches && IsSwitch(arg, &switch_string, &switch_value)) {
      ...
      if (has_single_argument_switch_) {
        ...
        params.append(base::StrCat(
            {kSwitchPrefixes[0], kSingleArgument, (" ")}));
      } else {
...
void CommandLine::ParseAsSingleArgument(
  ...
  (!raw_command_line_string_.empty());
  ...
  _NE(single_arg_switch_position, StringType::npos);
