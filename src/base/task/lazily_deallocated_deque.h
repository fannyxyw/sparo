...
using TimeTicks = std::chrono::steady_clock::time_point;
template <typename T, TimeTicks (*now_source)() = std::chrono::steady_clock::now>
class LazilyDeallocatedDeque {
 public:
  ...
  void MaybeShrinkQueue() {
    ...
    next_resize_time_ = current_time + std::chrono::seconds(kMinimumShrinkIntervalInSeconds);
  }
